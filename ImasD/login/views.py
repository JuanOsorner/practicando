...
from django.contrib.auth import (
    ...
    login,
    logout,
)  # Para autenticar y loguear, desloguear
...
def logout_views(request: HttpRequest) -> HttpResponse:
    ...
    """
    # Django trae por defecto el logout para cerrar las sesiones
    logout(request)
    # Redireccionamos al usuario a la vista de login
    return redirect("login:login")


def procesar_formulario(request: HttpRequest) -> HttpResponse:
    """
    ...
    if request.method == "POST":
        ...
        if user is not None:
            ...
            if user.tipo == "sa":
                return mensajes.json_mensaje(
                    "Bienvenido al sistema de super administracion de ImasD",
                    True,
                    "prueba.html",
                )
            elif user.tipo == "ad":
                return mensajes.json_mensaje(
                    "Bienvenido al sistema de administracion de ImasD",
                    True,
                    "prueba.html",
                )
            elif user.tipo == "us":
            ...
            return mensajes.json_mensaje(
                "No se pudo iniciar sesion, acerquese a un administrador para mas informacion",
                False,
                "login.html",
            )
    else:
